...
const gameInstance = (function () {
    ...
    const callbacks = {onturnchange: null, onmarkplace: null, ongamewin: null, ongametie: null}
    let currentPlayer;
    ...
    const playGame = () => {
        ...
        if(callbacks.onturnchange) callbacks.onturnchange(currentPlayer == players[1] ? 1 : 2);
        inputLocked = false;
    ...
    const playTurn = (row, col) => {
        ...
        if(callbacks.onmarkplace) callbacks.onmarkplace(currentPlayer.getInfo().mark, row, col);
    ...
    const checkWin = (row, col) => {
        ...
        const diagWin = gameBoard.board[1][1] == currentMark && 
            ((gameBoard.board[0][0] == currentMark && gameBoard.board[2][2] == currentMark) ||
            (gameBoard.board[0][2] == currentMark && gameBoard.board[2][0] == currentMark));
    ...
    const getWaiting = () => {return inputLocked;}
...
const gameInterface = (function () {
    ...
    updatePlayersInfo();
    gameInstance.setCallbacks({onmarkplace: placeMark, ongametie: onGameTie, onturnchange: onTurnChange, ongamewin: onGameWin})
    ...
    function notify(icon = 'iconWarn', message = []){
        ...
        if(!message[0] || message[0] == ''){
            const p = document.createElement('p'); p.textContent = 'Uh oh. Someone forgot to put a notification message here.';
            para.append(p);
        }else{
            message.forEach(v => {
    ...
    function updatePlayersInfo(){
        ...
        elements.p1Mark.textContent = gameInstance.players[1].getInfo().mark.toUpperCase();
        elements.p2Name.textContent = gameInstance.players[2].getInfo().name;
        elements.p2Mark.textContent = gameInstance.players[2].getInfo().mark.toUpperCase();
        resetScores();
    }
    function onGameTie(){
    ...
    function placeMark(mark, row, col){
        ...
        const svg = elements.displayBoard.querySelector(`[data-row="${row}"][data-col="${col}"] svg`);
    ...
    function moveHighlight(playerNum, tie = false){
        ...
        elements.highlight.style.width = tie ? `${outerRect.width}px` : `${infoRect.width}px`;
        elements.highlight.style.height = `${infoRect.height}px`;
        elements.highlight.style.transform = tie ? 'translate(0, 0)' : `translate(${left}px, ${top}px)`;
...
function createPlayer(playerType, playerName, playerMark){
    ...
    const setMark = (newMark) => {
        ...
        else throw new RangeError("Error in createPlayer().setMark() - Requires mark to be 'x' or 'o'");
    }

    const setName = (newName) => {
        if(newName.length >= 3) name = newName;
        else throw new RangeError("Error in createPlayer().setName() - Requires name to be at least 3 chars");
    }

    const setType = (newType) => {
        if(newType == 'npc' || newType == 'human') type = newType;
        else{
            throw new RangeError("Error in createPlayer().setType() - Requires type " + 
            `to be 'npc' or 'human'. Given: [Type: ${newType}:${typeof newType}]`);
        };
    };

    const getInfo = () => {return {type, name, mark}};
